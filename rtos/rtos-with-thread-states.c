...
int main()
{
    ...
    while (1)
    {
        ...
        // Suspend Thread 1 after 5 iterations
        if (thread_blocks[current_thread].thread_id == 0 && thread_blocks[current_thread].waiting < 5)
        {
            thread_blocks[current_thread].waiting++;
        }
        else if (thread_blocks[current_thread].thread_id == 0 && thread_blocks[current_thread].waiting == 5)
        {
            suspend_thread(current_thread);
        }

        // Terminate Thread 2 after 10 iterations
        if (thread_blocks[current_thread].thread_id == 1 && thread_blocks[current_thread].waiting < 10)
        {
            thread_blocks[current_thread].waiting++;
        }
        else if (thread_blocks[current_thread].thread_id == 1 && thread_blocks[current_thread].waiting == 10)
        {
...
// Thread 1 will be suspended after 5 iterations
// Thread 2 will be terminated after 10 iterations. 
// The thread state will be printed accordingly in the main loop.
